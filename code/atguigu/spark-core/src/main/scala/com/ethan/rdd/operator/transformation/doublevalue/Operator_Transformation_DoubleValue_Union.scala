 ...
 * @Version 1.0
 */
object Operator_Transformation_DoubleValue_Union {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setMaster("local[*]").setAppName("Operator_Transformation_DoubleValue_Union")
...
//    rdd01.union(rdd03).collect().foreach(println)
